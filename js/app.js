...
// ЖЕСТКО ВШИТЫЙ ПЕРСОНАЛ (СОВЕТ ПРИМЕНЕН ПРАВИЛЬНО)
const STAFF_DB = [
...
const app = {
    ...
    init: async () => {
        ...
        try {
            ...
            console.error("Supabase Load Info:", error);
        }
        ...
        setInterval(() => {
            ...
            if (currentHour >= 22 || currentHour < 6) document.body.classList.add('night-mode');
    ...
    renderStaff: () => {
        ...
        if(!sel) return;
        // Читаем из вшитого списка мгновенно
        ...
        $('btn-login-submit').onclick = () => {
            ...
            if (pinInput === correctPin) {
                ...
                app.ui.toast('Авторизация успешна', 'success'); app.checkSession();
    ...
    logIncident: () => {
        ...
        app.ui.toast('Инцидент зафиксирован', 'danger'); app.closeModals();
    },
    ...
    tables: {
        render: () => {
            ...
            grid.innerHTML = app.state.tables.sort((a,b)=>a.id-b.id).map(t => {
                ...
                return `<div class="table-card ${cls}"><div class="table-cloth"></div><div class="table-content flex-column h-100">
                    ...
                    ${t.bar_amount > 0 ? `<div class="muted-text text-10 mt-5">БАР: ${t.bar_amount.toLocaleString()} ₸</div>` : ''}` : `<div class="t-idle-text muted-text">СВОБОДЕН</div>`}</div>
        ...
        startTable: async (id) => {
            ...
            try {
                ...
                app.ui.toast(`Стол ${id} запущен`, 'success'); app.logActivity(`Запущен Стол ${id}`, '🟢');
            } catch(e) {}
        },
        togglePause: async (id) => {
            ...
            try {
                if (t.paused) {
                    ...
                    app.ui.toast(`Пауза`, 'warning'); app.logActivity(`Пауза: Стол ${id}`, '⏸');
                }
            } catch(e) {}
        },
        openManage: (id) => {
        ...
        confirmStopTable: async () => {
            ...
            try {
                ...
                app.closeModals(); app.ui.playSound('pay'); app.ui.toast(`Счет передан в кассу`, 'success'); app.logActivity(`Расчет: Стол ${id}`, '⏹');
            } catch(e) {}
        },
        getCost: (t) => {
    ...
    pos: {
        ...
        checkout: async (method) => {
            ...
            try {
                ...
                app.ui.toast(`Продажа: ${method}`, 'success'); app.logActivity(`POS: ${total} ₸ (${method})`, '🍹');
        ...
        sendToTable: async () => {
            ...
            try {
                ...
                app.ui.toast(`На Стол ${tableId}`, 'success'); app.state.cart = []; app.pos.renderCart(); app.refreshInventoryCache(); app.switchTab('hall');
            } catch(e) {}
        }
    },
    ...
    renderChecks: () => {
        ...
        if (app.state.activeChecks.length === 0) { list.innerHTML = '<div class="muted-text py-10 w-100 text-center">Счета оплачены ✅</div>'; return; }
        list.innerHTML = app.state.activeChecks.map(c => `
            <div class="payment-row">
                <div class="payment-info"><span class="badge" style="background: rgba(255,255,255,0.05);">🎱 Ст. ${c.table_id}</span><b class="text-white text-12">${c.guest_name}</b></div>
    ...
    confirmPayActiveCheck: async (method) => {
        ...
        try {
            ...
            app.closeModals(); app.ui.toast(`Оплата ${method}`, 'success'); app.logActivity(`Касса закрыта (${method})`, '🟢');
